...
> **60-second summary**
> - Ubuntu/WSL-focused minimal client/server that perform a TLS handshake, then a **HELO → POW → info requests → END** flow.
...
- performs a simple line-based handshake (`HELO` then `POW`)
- solves a Proof-of-Work challenge where the client must find a suffix so that `SHA1(authdata + suffix)` has 
a required number of **leading hex zeros**
- answers a sequence of server “info request” commands until `END` (or `ERROR`).

The hasher (SHA1) is an explicit constraint from the challenge and **must not be changed** in
this project.

The POW difficulty is configurable (see `--difficulty` on the server). The challenge target
...
- **C++ POW solver**: CMake **≥ 3.16**, a **C++20** compiler (GCC/Clang), and OpenSSL dev libs (`libssl-dev`)
...
```bash
# Terminal 1
python -m tlscc.server --tutorial

# Terminal 2
python -m tlscc.client --tutorial
...
- `XDG_STATE_HOME/tlscc/logs/`

If `XDG_STATE_HOME` is not set, it defaults to:

- `~/.local/state/tlscc/logs/`

So the default log files are:

- `~/.local/state/tlscc/logs/server.log`
- `~/.local/state/tlscc/logs/client.log`
