...
checks are not available in Windows (such as checking that the WORK benchmark binary file launched with 
subprocess.run is not world writable).  

To run TLS line protocol, fire up a terminal window and run the following command in the "src" folder:
```sh
$ python server.py
```
