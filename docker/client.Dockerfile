...
# For building or running the POW helper:
...
RUN mkdir -p /app/src/tlscc/_bin
COPY --from=builder /app/build/pow_challenge /app/src/tlscc/_bin/pow_challenge
RUN chmod +x /app/src/tlscc/_bin/pow_challenge
