...
def send_and_receive(token: str, to_send: str, secure_sock: socket.socket) -> None:
    """Send message and receive the string from the client.
    ...
    try:
        ...
        if to_send.startswith("WORK"):
            ...
            print(
                f"WORK suffix from client: {suffix}\n"
...
def main() -> int:
    ...
    while True:
        ...
        with context.wrap_socket(client_socket, server_side=True) as secure_sock:
            ...
            # handshake
            send_and_receive(token, "HELLO", secure_sock)
            ...
            for _ in range(20):
                ...
                send_and_receive(token, f"{choice} " f"{random_string}", secure_sock)

            # end message
            send_and_receive(token, "DONE", secure_sock)
            break
