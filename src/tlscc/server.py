...
2) Performs a small handshake:
   - Sends ``HELO`` and expects ``EHLO``.
   - Sends ``POW <authdata> <difficulty>`` and expects a suffix that yields a SHA1
   hash with the required number of leading zeros.
3) Sends a sequence of body requests (e.g., ``NAME``, ``MAILNUM``, ...). The client
   responds with ``<cksum> <value>`` where ``cksum = SHA1(authdata + random_string)``.
4) Finishes with ``END`` and expects ``OK``.

Timeouts:
- POW has a long timeout (default: 2 hours).
...
def build_server_parser() -> argparse.ArgumentParser:
    """Build the CLI argument parser for ``tlscc-server``.
    ...
    Examples:
        ...
        >>> ns = p.parse_args(["--port", "3481", "--tutorial"])
        >>> ns.port
        3481
    """
    parser = argparse.ArgumentParser(
        prog="tlscc-server", description="TLS server for the coding challenge."
    )
...
def _check_suffix(to_send: str, authdata: str, received_message: str) -> bool:
    """Validate a POW suffix against the requested difficulty.

    Args:
        to_send (str): The original POW command string (``"POW <authdata> <difficulty>"``).
        authdata (str): Authentication data used as the hash prefix.
        received_message (str): Suffix returned by the client.

    Returns:
        (bool): True if ``SHA1(authdata + suffix)`` starts with ``difficulty`` leading ``"0"``.
    """
...
def _check_cksum(to_send: str, authdata: str, received_message: str) -> bool:
    ...
    ``"<cksum> <value>"`` where ``cksum = SHA1(authdata + random_string)``.
...
def send_and_receive(
    ...
    - Sends ``to_send`` using ``tlscc.protocol.send_message``.
    - Receives a single-line reply using ``tlscc.protocol.receive_message``.
    - For ``POW`` commands, validates the suffix difficulty requirement.
    - For body commands, validates the checksum.
    - For ``ERROR`` commands, no reply is expected (returns ``""``).

    Args:
        authdata (str): Authentication data used for checksum/suffix validation.
    ...
    Raises:
        ...
        ValueError: If POW suffix or checksum validation fails.

    Examples:
        >>> import socket
        >>> from tlscc.server import send_and_receive
        >>> a, b = socket.socketpair()
        >>> try:
        ...     # peer responds to HELO with EHLO
        ...     _ = b.recv(1024)  # consume HELO (sent by send_and_receive)
        ... except Exception:
        ...     pass
        ... finally:
        ...     a.close(); b.close()
...
def handle_one_session(
    ...
    Runs the handshake (HELO + POW), then sends a series of body requests, and
    finishes with ``END`` unless an ``ERROR`` condition is triggered.
    ...
    msg = send_and_receive(cfg.authdata, "HELO", secure_sock, cfg.other_timeout)
    logger.debug(f"Received {msg!r}")

    # only log first part of authdata
    half_len_authdata = math.ceil(len(cfg.authdata) / 2)
    logger.debug(
        f"Authentication data: {cfg.authdata[:half_len_authdata]!r}..., Difficulty: {cfg.difficulty!r}"
    )

    msg = send_and_receive(
        cfg.authdata,
        "POW " + str(cfg.authdata) + " " + str(cfg.difficulty),
        secure_sock,
        cfg.pow_timeout,
    )
    logger.debug(f"Valid suffix returned from client: {msg!r}")
    this_hash = hashlib.sha1((cfg.authdata + msg).encode()).hexdigest()  # noqa: S324
    ...
    if choice is not None and not choice.startswith("ERROR"):
        ...
        msg = send_and_receive(cfg.authdata, "END", secure_sock, cfg.other_timeout)
        logger.info(f"Received {msg!r}")


def main(argv: Sequence[str] | None = None) -> int:
    """Run the ``tlscc-server`` CLI.
