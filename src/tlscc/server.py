...
def _check_suffix(to_send: str, authdata: str, received_message: str) -> bool:
    ...
    Args:
        to_send: The original POW command string (``"POW <authdata> <difficulty>"``).
        authdata: Authentication data used as the hash prefix.
        received_message: Suffix returned by the client.

    Returns:
        True if ``SHA1(authdata + suffix)`` starts with ``difficulty`` leading ``"0"``.
    """
...
def send_and_receive(
    ...
    Args:
        authdata: Authentication data used for checksum/suffix validation.
