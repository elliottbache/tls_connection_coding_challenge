...
class TestHasher:
    def test_hasher(self, token, random_string):
        assert (
            hashlib.sha256((token + random_string).encode()).hexdigest()  # noqa: S324
...
class TestRunPowBinary:
    def test_run_pow_binary_success(self, fake_bin, token, difficulty,
    ...
    def test_run_pow_binary_non_str_token(self, fake_bin, difficulty,
    ...
    def test_run_pow_binary_invalid_token(self, difficulty,
                                             fake_bin):
        wrong_token = 'poiasfdlkas+/'
        with pytest.raises(ValueError, match="token contains disallowed characters or length"):
            client.run_pow_binary(fake_bin, wrong_token, difficulty)

        wrong_token = 'poiasfdlkaspppppppppppppppppppppppppppppppppppppppppppppppp'\
            + 'asdfdddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd' \
            + 'asdfdddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd'
        with pytest.raises(ValueError, match="token contains disallowed characters or length"):
            client.run_pow_binary(fake_bin, wrong_token, difficulty)

    def test_run_pow_binary_non_int_difficulty(self, token,
                                               fake_bin):
    ...
    def test_run_pow_binary_invalid_difficulty(self, token,
                                               fake_bin):
        wrong_difficulty = 65
        with pytest.raises(ValueError, match="WORK difficulty is out of range"):
            client.run_pow_binary(fake_bin, token, wrong_difficulty)

    def test_run_pow_binary_no_executable(self, token, difficulty,
                                          fake_bin):
    ...
    def test_run_pow_binary_writable_linux_executable(self, token, difficulty,
                                          fake_bin):
    ...
    def test_run_pow_binary_error(self, token, difficulty, suffix,
...
class TestHandlePowCpp:
    def test_handle_pow_cpp_success(self, fake_bin, token, difficulty, timeout,
                                    suffix, pow_hash, path_to_pow_benchmark,
                                    monkeypatch, readout):
        ...
        err, suffix_output = client.handle_pow_cpp(token, difficulty,
                                      str(fake_bin))
    ...
    def test_handle_pow_cpp_no_executable(self, token, difficulty, timeout,
                                          path_to_pow_benchmark):

        with pytest.raises(FileNotFoundError, match="WORK binary not a regular file"):
            client.handle_pow_cpp(token, difficulty, path_to_pow_benchmark, timeout)

    def test_handle_pow_cpp_no_result(self, token, difficulty, timeout,
                                      fake_bin, monkeypatch):
...
class TestConnectToServer:
    ...
    @pytest.mark.parametrize("exc, expected", [
        ...
        (lambda: ConnectionRefusedError(),
         "Connection refused by localhost:1234"),
