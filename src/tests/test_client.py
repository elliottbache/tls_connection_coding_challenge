...
def test_hasher(token, random_string):
    assert (hashlib.sha256((token + random_string).encode()).hexdigest()
            == client.hasher(token, random_string))
...
def test_handle_pow_cpp_success(token, difficulty, threads, suffix, pow_hash, path_to_pow_benchmark, monkeypatch, capsys):
    def fake_subprocess_run(*a, **k):
        return SimpleNamespace(stdout="RESULT:" + suffix + "\n", stderr="", returncode=0)

    monkeypatch.setattr(client.subprocess, "run", fake_subprocess_run)

    assert client.handle_pow_cpp(token, difficulty, path_to_pow_benchmark, threads) == (0, (suffix + '\n').encode())

    captured = capsys.readouterr().out
    assert f"Authdata: {token}\nValid WORK Suffix: {suffix}\n" \
           f"Hash: {pow_hash}" in captured

def test_handle_pow_cpp_non_str_token(token, difficulty, threads, path_to_pow_benchmark, capsys):
    wrong_token = 5.3

    assert client.handle_pow_cpp(wrong_token, difficulty, path_to_pow_benchmark, threads) == (4, '\n'.encode())

    captured = capsys.readouterr().out
    assert "token is not a string.  Exiting since hashing function " \
              "will not work correctly" in captured

def test_handle_pow_cpp_non_int_difficulty(token, difficulty, threads, path_to_pow_benchmark, capsys):
    wrong_difficulty = 'five'

    assert client.handle_pow_cpp(token, wrong_difficulty, path_to_pow_benchmark, threads) == (4, '\n'.encode())

    captured = capsys.readouterr().out
    assert "WORK difficulty is not an integer" in captured

def test_handle_pow_cpp_no_executable(token, difficulty, threads, path_to_pow_benchmark, capsys):

    assert client.handle_pow_cpp(token, difficulty, path_to_pow_benchmark, threads) == (4, '\n'.encode())

    captured = capsys.readouterr().out
    assert "WORK benchmark executable not found." in captured

def test_handle_pow_cpp_error(token, difficulty, threads, suffix, pow_hash, path_to_pow_benchmark, monkeypatch, capsys):
    def fake_subprocess_run(*a, **k):
...
def test_handle_pow_cpp_no_result(token, difficulty, threads, suffix, pow_hash, path_to_pow_benchmark, monkeypatch, capsys):
    def fake_subprocess_run(*a, **k):
...
def test_define_response_success_helo(token, valid_messages, path_to_pow_benchmark, threads, capsys):


    q = queue.Queue()
    responses = {}
    client.define_response(["HELLO"], token, valid_messages, q, responses, path_to_pow_benchmark, threads)
    assert q.get() == [0, "HELLOBACK\n".encode()]

    captured = capsys.readouterr().out
    assert norm(captured) == ""

def test_define_response_success_end(token, valid_messages, path_to_pow_benchmark, threads, capsys):
    import queue

    q = queue.Queue()
    responses = {}
    client.define_response(["DONE"], token, valid_messages, q, responses, path_to_pow_benchmark, threads)
    assert q.get() == [1, "OK\n".encode()]

    captured = capsys.readouterr().out
    assert norm(captured) == ""

def test_define_response_success_error(token, valid_messages, path_to_pow_benchmark, threads, capsys):
    import queue

    q = queue.Queue()
    responses = {}
    client.define_response(["ERROR", "test", "args"], token, valid_messages, q, responses, path_to_pow_benchmark, threads)
    assert q.get() == [2, "\n".encode()]

    captured = capsys.readouterr().out
    assert norm(captured) == "Server error: test args"

def test_define_response_success(token, random_string, valid_messages, path_to_pow_benchmark, threads, capsys):
    import queue
    q = queue.Queue()
    responses = {"MAILNUM": "2"}
    args = ["MAILNUM", random_string]

    client.define_response(args, token, valid_messages, q, responses, path_to_pow_benchmark, threads)
    out_string = hashlib.sha256((token + args[1]).encode()).hexdigest() + " " + responses[args[0]] + "\n"
    assert q.get() == [0, out_string.encode()]

    captured = capsys.readouterr().out
    assert "Extra arguments = " + args[1] + "\nAuthentification data = " +  token in norm(captured)

def test_define_response_success_pow(token, difficulty, random_string, suffix, valid_messages, path_to_pow_benchmark, threads, capsys, monkeypatch):
    import queue
    q = queue.Queue()
    responses = {"MAILNUM": "2"}
    ...
    captured = capsys.readouterr().out
    assert "The time of execution of WORK challenge is :" in norm(captured)

def test_define_response_success_invalid(token, valid_messages, path_to_pow_benchmark, threads, capsys):
    import queue

    q = queue.Queue()
    responses = {}
    client.define_response(["HELLOP"], token, valid_messages, q, responses, path_to_pow_benchmark, threads)
    assert q.get() == [4, "\n".encode()]

    captured = capsys.readouterr().out
    assert norm(captured) == ""

def test_define_response_result_no_newline(token, difficulty, random_string, suffix, valid_messages, path_to_pow_benchmark, threads, capsys, monkeypatch):
    import queue
    q = queue.Queue()
    responses = {"MAILNUM": "2"}
    args = ["WORK", token, difficulty]
...
def test_connect_to_server_success(capsys):
    ...
    captured = capsys.readouterr().out
    assert "Connected to 1234" in norm(captured)

@pytest.mark.parametrize("exc, expected", [
    (lambda: socket.timeout(), "Connect timeout to localhost:1234"),
